...
bool RadioMedium::areReceiverAndTransmitterSatellites(const Radio* receiverRadio, const ITransmission *transmission) const
{
    ...
    // Accept derived satellite module types too (e.g., mcast.McastSatellite)
    if (transmitterModuleType.find("Satellite") == std::string::npos) {
        return false;
    }
    ...
    if (receiverModuleType.find("Satellite") == std::string::npos) {
        return false;
    }
    return true;
}
...
bool RadioMedium::isElevationAngleLargeEnough(const Radio *receiverRadio, const ITransmission *transmission, const IArrival *arrival) const
{
    ...
    // Accept derived module types by substring matching (e.g., mcast.McastSatellite)
    if (receiverModuleType.find("SatDrone") != std::string::npos || transmitterModuleType.find("SatDrone") != std::string::npos) {
        minElevationAngle = mediumLimitCache->getMinElevationAngleD2S();
    } else if (receiverModuleType.find("SatCar") != std::string::npos || transmitterModuleType.find("SatCar") != std::string::npos) {
    ...
    // satellite may be a derived NED type (e.g. mcast.McastSatellite), accept those too
    if (transmitterModuleType.find("Satellite") != std::string::npos) {
        // satellite is transmitter
        direction = transmission->getStartPosition() - arrival->getEndPosition();
    }
    else if (receiverModuleType.find("Satellite") != std::string::npos) {
